...
module.exports = (bot) => {
    ...
    app.get('/webapp', (req, res) => {
        ...
        h += '<header class="header glass"><div><h2 id="greet">Hello</h2><p id="v-title">Discover</p></div><div class="top-bal" onclick="switchT(\'deposit\')"><span id="u-bal">0đ</span></div></header>';
        h += '<div id="view-discover" class="view active"><div class="search"><i class="fa-solid fa-magnifying-glass"></i><input type="text" id="q" placeholder="Search premium assets..." oninput="onQ()"></div><div class="cat-scroll"><div class="cat-pill active" data-id="all" onclick="onC(\'all\')">✨ Discover</div>' + catsHtml + '</div><div id="plist">' + prodsHtml + '</div></div>';
        h += '<div id="view-history" class="view"><div id="hlist"><p style="text-align:center; opacity:0.3; padding:100px 0;">No records found.</p></div></div>';
        h += '<div id="view-deposit" class="view"><h3>Add Credits</h3><div class="amt-grid"><div class="amt-btn glass" onclick="setA(50000)">50k</div><div class="amt-btn glass" onclick="setA(100000)">100k</div><div class="amt-btn glass" onclick="setA(200000)">200k</div><div class="amt-btn glass" onclick="setA(500000)">500k</div></div><input type="number" id="ain" style="width:100%; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); border-radius:24px; padding:24px; color:#fff; font-size:28px; font-weight:900; text-align:center; margin-bottom:32px;" placeholder="0đ"><button style="width:100%; background:var(--p); color:#fff; border:none; border-radius:24px; padding:20px; font-size:18px; font-weight:800; box-shadow:0 15px 30px rgba(0,122,255,0.4);" onclick="pay()">Secure Payment</button><div id="qrw" class="glass" style="margin-top:40px; padding:32px; border-radius:40px; text-align:center; display:none;"><img id="qri" style="width:100%; max-width:240px; border-radius:20px; margin-bottom:20px; border:8px solid #fff;"><div id="qri-info" style="font-size:15px; line-height:1.8;"></div></div></div>';
        h += '<div id="view-profile" class="view"><div class="prof-hero glass"><div class="avatar" id="u-av">?</div><div id="u-name" style="font-size:28px; font-weight:900;">User</div><div style="display:flex; background:rgba(255,255,255,0.1); border-radius:24px; overflow:hidden; margin-top:32px;"><div style="flex:1; padding:20px; background:rgba(0,0,0,0.25); border-right:1px solid rgba(255,255,255,0.1);"><span id="s-ord" style="display:block; font-size:20px; font-weight:900;">0</span>Orders</div><div style="flex:1; padding:20px; background:rgba(0,0,0,0.25);"><span id="s-spt" style="display:block; font-size:20px; font-weight:900;">0đ</span>Spent</div></div></div><div style="padding:0 16px;"><p style="font-size:12px; font-weight:800; color:rgba(255,255,255,0.4); text-transform:uppercase;">Coupon Code</p><div style="display:flex; gap:12px; margin-top:12px;"><input type="text" id="p-code" style="width:100%; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); border-radius:20px; padding:18px 24px; color:#fff; font-size:16px; font-weight:600;"><button class="btn" style="height:60px;" onclick="promo()">Apply</button></div><p style="font-size:12px; font-weight:800; color:rgba(255,255,255,0.4); text-transform:uppercase; margin-top:24px;">Support Channel</p><textarea id="s-msg" style="width:100%; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); border-radius:20px; padding:18px 24px; color:#fff; font-size:15px; height:100px; resize:none; margin-top:12px;"></textarea><button style="width:100%; background:var(--s); color:#fff; border:none; border-radius:24px; padding:20px; font-size:18px; font-weight:800; margin-top:16px;" onclick="ask()">Send to Admins</button></div></div>';
        ...
        h += '<div id="modal" class="modal"><div class="m-bg" onclick="closeM()"></div><div class="m-box glass"><h3>Item Details</h3><div id="m-b" class="k-box"></div><button style="width:100%; background:#fff; color:#000; border:none; border-radius:20px; padding:16px; font-weight:900;" onclick="closeM()">Got it</button></div></div>';
        ...
        h += 'if(userId) { const u = tg.initDataUnsafe.user; document.getElementById("greet").innerText = "Hi, " + u.first_name; document.getElementById("u-name").innerText = u.first_name; document.getElementById("u-av").innerText = u.first_name.charAt(0); sync(); }';
        h += 'function switchT(t) { document.querySelectorAll(".view").forEach(v => v.classList.remove("active")); document.querySelectorAll(".dock-item").forEach(i => i.classList.remove("active")); document.getElementById("view-" + t).classList.add("active"); document.getElementById("t-" + t).classList.add("active"); document.getElementById("v-title").innerText = t.charAt(0).toUpperCase() + t.slice(1); if(t==="history") loadH(); if(t==="profile") sync(); }';
        h += 'async function sync() { if(!userId) return; try { const r = await fetch("/api/webapp/user/" + userId); const d = await r.json(); document.getElementById("u-bal").innerText = new Intl.NumberFormat("vi-VN").format(d.balance) + "đ"; document.getElementById("s-ord").innerText = d.orders; document.getElementById("s-spt").innerText = new Intl.NumberFormat("vi-VN").format(d.spent) + "đ"; } catch(e) {} }';
        h += 'async function loadH() { const l = document.getElementById("hlist"); try { const r = await fetch("/api/webapp/history/" + userId); const orders = await r.json(); if(orders.length === 0) { l.innerHTML = "<p style=\'opacity:0.3; padding:100px 0; text-align:center;\'>Empty history.</p>"; return; } l.innerHTML = orders.map(o => { return \'<div class="glass" style="padding:24px; border-radius:28px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;"><div><b style="font-size:17px;">\' + o.product_name + \'</b><br><small style="opacity:0.4; text-transform:uppercase; font-weight:800; font-size:10px;">\' + o.status + \'</small></div>\' + (o.status==="delivered" ? \'<button style="background:#fff; color:#000; border:none; border-radius:12px; padding:10px 20px; font-weight:800; font-size:13px;" onclick="showK(\\\'\' + o.keys.join(\'\\\\n\') + \'\\\')">Keys</button>\' : \'<b style="font-size:16px;">\' + new Intl.NumberFormat("vi-VN").format(o.total_price) + \'đ</b>\') + \'</div>\'; }).join(""); } catch(e) { l.innerHTML = "Error."; } }';
        ...
        h += 'function setA(a) { document.getElementById("ain").value = a; pay(); } async function pay() { const a = document.getElementById("ain").value; try { const r = await fetch("/api/webapp/payment/create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId, amount: a }) }); const d = await r.json(); document.getElementById("qri").src = d.qrUrl; document.getElementById("qri-info").innerHTML = "Transfer: <b>" + d.bankName + "</b><br>Content: <b>" + d.paymentCode + "</b>"; document.getElementById("qrw").style.display = "block"; } catch(e) { tg.showAlert("Error"); } }';
        h += 'async function promo() { const c = document.getElementById("p-code").value; try { const r = await fetch("/api/webapp/promo/apply", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId, code: c }) }); const res = await r.json(); tg.showAlert(res.msg); if(res.success) { document.getElementById("p-code").value=""; sync(); } } catch(e) { tg.showAlert("Error"); } }';
        h += 'async function ask() { const m = document.getElementById("s-msg").value; try { await fetch("/api/webapp/support/send", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId, message: m, fullName: tg.initDataUnsafe.user.first_name }) }); tg.showAlert("Sent!"); document.getElementById("s-msg").value=""; } catch(e) { tg.showAlert("Error"); } }';
        h += 'function buy(id) { tg.openTelegramLink("https://t.me/" + bname + "?start=buy_" + id); tg.close(); } function showK(k) { document.getElementById("m-b").innerText = k; document.getElementById("modal").style.display = "flex"; } function closeM() { document.getElementById("modal").style.display = "none"; }</script></body></html>';
        res.send(h);
    });
